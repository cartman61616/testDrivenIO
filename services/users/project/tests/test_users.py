...
class TestUserService(BaseTestCase):
    ...
    def test_single_user(self):
        ...
        user = add_user('dicky', 'otis@bosstones.com')
        db.session.add(user)
        db.session.commit()
        with self.client:
    ...
    def test_all_users(self):
        ...
        add_user('robo', 'atomic_robo@tesladyne.com')
        add_user('marty', 'goalie30@devils.com')
        with self.client:
            ...
            self.assertIn('atomic_robo@tesladyne.com', data['data']['users'][0]['email'])
            self.assertIn('marty', data['data']['users'][1]['username'])
            self.assertIn('goalie30@devils.com', data['data']['users'][1]['email'])
            self.assertIn('success', data['status'])
    ...
    def test_main_add_user(self):
        ...
        with self.client:
            response = self.client.post(
                '/',
                data=dict(username='dr dinosaur', email='dino@tesladyne.com'),
